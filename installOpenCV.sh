...
#
# this builkd uses TBB, NEON, VFPV3 for raspi optimization
# For 4.1.1 you need to add -latomic and precompiled headers off and c++11 standard with these flags
...
# cofigure & generate
make -j3
# A 1Gb raspberry py 3 is getting pushed on its limits with this build
# You need to increase swap space (in zram) and execute last compile step with single proc -j1 if build does not complete
sudo make install
sudo ldconfig
